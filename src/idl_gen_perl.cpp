...
class PerlGenerator : public BaseGenerator {
 public:
  PerlGenerator(const Parser &parser, const std::string &path,
 ...
 private:
  ...
  bool SaveType(const Definition &def, const std::string &classcode,
    ...
    code += classcode;
    std::string filename =
        NamespaceDir(*def.defined_namespace) + NormalizedName(def) + ".pl";
...
bool GeneratePerl(const Parser &parser, const std::string &path,
                  const std::string &file_name) {
  perl::PerlGenerator generator(parser, path, file_name);
